...
module CTCI
  module C7
    module P5
      ...
      class Square
        @sqr = Struct.new(:top_left, :top_right, :bottom_left, :bottom_right)
      ...
      def point_within_segment?(point, segment_start, segment_end)
        within_x = (segment_start.x..segment_end.x).cover?(point.x)
        within_y = (segment_start.y..segment_end.y).cover?(point.y)
        within_x && within_y
      end
